...
def mobility_bronze_silver():
   ...
   # -------------------- MUNICIPIOS --------------------
    ...
    def get_year_2023_files():
        return [
            {'tabla': 'bronze.trips_distritos_2023', 'tipo': 'distritos'},
            {'tabla': 'bronze.trips_municipios_2023', 'tipo': 'municipios'},
            {'tabla': 'bronze.trips_gaus_2023', 'tipo': 'gaus'}
    ]
    ...
    def populate_viajes_table(config: dict): 
        ...
        print(f"🚀 Procesando tabla bronze: {tabla} (tipo: {tipo})")
        ...
        con.execute(f"DELETE FROM silver.viajes WHERE tipo_zona = '{tipo}';")

        print(f"📥 Insertando datos de {tabla} transformando {mitma_col} a {ine_col}...")
        ...
        con.close()
        print(f"✅ Finalizada carga completa de {tipo} desde {tabla}")
